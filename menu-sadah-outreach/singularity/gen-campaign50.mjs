...
const rows = cafes.map((c, i) => `<div class="row"><div class="top"><b>#${i + 1}</b> <span class="nm">${esc(c.name)}</span> <span class="ar">${esc(c.nameAr || '')}</span>
...
<textarea readonly id="a${i}">${esc(dmAr(c))}</textarea>
...
<textarea readonly id="n${i}" style="display:none">${esc(nudgeAr(c))}</textarea>
<button class="mini" onclick="cp('n${i}',this)">↻ Day-2</button>
<textarea readonly id="b${i}" style="display:none">${esc(byeAr(c))}</textarea>
<button class="mini" onclick="cp('b${i}',this)">🕊 Day-5</button></div>`).join('\n');
