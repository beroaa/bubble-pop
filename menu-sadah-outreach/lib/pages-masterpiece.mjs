...
export function masterpieceMenuPage(cafe, brief) {
  ...
  // WAVE 3 CART: every order chip is a +add control; the cart sheet handles the WhatsApp checkout
  const orderChip = (en, ar, price, id) =>
    `<button class="order-chip touchable" type="button" data-add data-id="${esc(id)}" data-ar="${esc(ar)}" data-en="${esc(en)}" data-price="${Number(price) || 0}" aria-label="${esc(ar)}"><span class="oc-t"><span class="ar">أضف</span><span class="en">Add</span></span><span class="oc-plus">＋</span><span class="oc-q" hidden></span></button>`;
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  /* ---- WAVE 3: CART (add chips → floating bar → slide-up sheet → WhatsApp checkout) ---- */
  .order-chip{position:relative}
  .order-chip .oc-plus{font-weight:800}
  .order-chip.incart{border-color:var(--accent);box-shadow:inset 0 0 0 2px var(--accent)}
  ${P.mode === 'light' ? '.order-chip.incart{box-shadow:2px 2px 0 var(--ink),inset 0 0 0 2px var(--accent)}' : ''}
  .order-chip.incart .oc-t,.order-chip.incart .oc-plus{display:none}
  .order-chip .oc-q{font-weight:800;font-variant-numeric:tabular-nums}
  .cartbar{position:fixed;inset-inline:0;bottom:0;z-index:45;display:flex;justify-content:center;
    padding:10px 14px calc(10px + env(safe-area-inset-bottom));transform:translateY(130%);
    transition:transform .3s cubic-bezier(.2,1,.3,1);pointer-events:none}
  .cartbar.show{transform:none}
  .cartbar button{pointer-events:auto;display:flex;align-items:center;justify-content:space-between;gap:12px;width:100%;max-width:460px;
    background:var(--ink);color:var(--bg-0);border:none;border-radius:16px;padding:14px 18px;font-family:inherit;font-weight:800;
    font-size:15px;cursor:pointer;box-shadow:0 10px 28px ${a}55,0 2px 0 ${a}}
  .cartbar .cb-l{display:flex;align-items:center;gap:10px}
  .cartbar .cb-count{background:var(--bg-0);color:var(--ink);border-radius:999px;min-width:23px;height:23px;
    display:grid;place-items:center;font-size:12.5px;padding:0 6px;font-variant-numeric:tabular-nums}
  .cartbar .cb-total{font-variant-numeric:tabular-nums;display:flex;align-items:center;gap:4px}
  .cartsheet{position:fixed;inset:0;z-index:70;visibility:hidden}
  .cartsheet.open{visibility:visible}
  .cartsheet .cs-back{position:absolute;inset:0;background:#00000088;opacity:0;transition:opacity .3s}
  .cartsheet.open .cs-back{opacity:1}
  .cartsheet .cs-panel{position:absolute;inset-inline:0;bottom:0;background:var(--bg-1);border-top:2px solid var(--ink);
    border-radius:22px 22px 0 0;max-height:84vh;display:flex;flex-direction:column;transform:translateY(100%);
    transition:transform .34s cubic-bezier(.2,1,.3,1);box-shadow:0 -12px 34px #00000055}
  .cartsheet.open .cs-panel{transform:none}
  .cs-head{display:flex;align-items:center;justify-content:space-between;padding:16px 18px 12px;border-bottom:1px solid var(--border-1)}
  .cs-head b{font-size:18px;font-weight:800;color:var(--text-1)}
  .cs-close{background:none;border:none;font-size:24px;color:var(--text-2);cursor:pointer;line-height:1;padding:2px 6px;font-family:inherit}
  .cs-list{overflow-y:auto;padding:4px 18px;flex:1;min-height:60px}
  .cs-row{display:flex;align-items:center;gap:12px;padding:13px 0;border-bottom:1px solid ${P.bg2}99}
  .cs-row:last-child{border-bottom:none}
  .cs-nm{flex:1;min-width:0;font-weight:600;font-size:16px;color:var(--text-1);line-height:1.3}
  .cs-pr{color:var(--sa);font-weight:800;font-variant-numeric:tabular-nums;white-space:nowrap;font-size:15px}
  .cs-step{display:flex;align-items:center;gap:9px;flex:0 0 auto}
  .cs-step button{width:31px;height:31px;border-radius:9px;border:2px solid var(--ink);background:var(--bg-0);color:var(--text-1);
    font-size:18px;font-weight:800;cursor:pointer;line-height:1;font-family:inherit;display:grid;place-items:center}
  .cs-step .cs-q{min-width:20px;text-align:center;font-weight:800;font-variant-numeric:tabular-nums;color:var(--text-1)}
  .cs-empty{text-align:center;color:var(--text-3);padding:34px 10px;font-size:15px}
  .cs-foot{padding:14px 18px calc(16px + env(safe-area-inset-bottom));border-top:2px solid var(--ink);background:var(--bg-1)}
  .cs-total{display:flex;justify-content:space-between;align-items:baseline;margin-bottom:12px;font-weight:800;font-size:19px;color:var(--text-1)}
  .cs-total .cs-tv{color:var(--sa);font-variant-numeric:tabular-nums}
  .cs-checkout{display:block;width:100%;text-align:center;background:#1faa53;color:#fff;border:2px solid var(--ink);border-radius:14px;
    padding:14px;font-family:inherit;font-weight:800;font-size:16px;cursor:pointer;text-decoration:none;box-shadow:3px 3px 0 var(--ink)}
  .cs-note{text-align:center;font-size:12px;color:var(--text-3);margin-top:9px;line-height:1.5}
  /* keep the corner floaters clear of the cart bar when it's up */
  body.cart-active .skydial,body.cart-active .wa{bottom:84px}
  body.cart-active .waiter-btn{bottom:84px}
  @media print{.cartbar,.cartsheet{display:none!important}}
...
<div class="cartbar" id="cartBar" aria-hidden="true">
  <button type="button" id="cartOpen" class="touchable">
    <span class="cb-l">🛒 <span class="ar">سلّتك</span><span class="en">Your cart</span> <span class="cb-count" id="cbCount">0</span></span>
    <span class="cb-total"><span id="cbTotal">0</span> ${SAR}</span>
  </button>
</div>
<div class="cartsheet" id="cartSheet" role="dialog" aria-modal="true" aria-label="Cart">
  <div class="cs-back" id="csBack"></div>
  <div class="cs-panel">
    <div class="cs-head"><b><span class="ar">سلّة طلبك</span><span class="en">Your order</span></b><button class="cs-close" id="csClose" type="button" aria-label="إغلاق">✕</button></div>
    <div class="cs-list" id="csList"></div>
    <div class="cs-foot">
      <div class="cs-total"><span><span class="ar">الإجمالي</span><span class="en">Total</span></span><span><span class="cs-tv" id="csTotal">0</span> ${SAR}</span></div>
      <a class="cs-checkout touchable" id="csCheckout" href="#" role="button"><span class="ar">أرسل الطلب على واتساب</span><span class="en">Send order on WhatsApp</span></a>
      <div class="cs-note" id="csNote"></div>
    </div>
  </div>
</div>
...
<script>
  ...
  // ---- WAVE 3: CART engine (localStorage, add/qty/remove, WhatsApp checkout) ----
  (function(){
    var SLUG=${JSON.stringify(cafe.slug)}, PHONE=${cafePhone ? JSON.stringify(cafePhone) : 'null'}, NM=${JSON.stringify(cafe.nameAr || cafe.name)}, CUR=${JSON.stringify(SAR)};
    var KEY='ms-cart-'+SLUG, cart={};
    try{cart=JSON.parse(localStorage.getItem(KEY))||{};}catch(e){cart={};}
    var bar=document.getElementById('cartBar'), sheet=document.getElementById('cartSheet');
    var cbCount=document.getElementById('cbCount'), cbTotal=document.getElementById('cbTotal');
    var csList=document.getElementById('csList'), csTotal=document.getElementById('csTotal'), csNote=document.getElementById('csNote'), csCheckout=document.getElementById('csCheckout');
    ...
    function orderText(){
      var ar=lang()==='ar', lines=[];
      for(var k in cart){var it=cart[k];lines.push('• '+(ar?it.ar:it.en)+' ×'+it.qty+' — '+fmt(it.qty*it.price)+' '+CUR);}
      var head=ar?("السلام عليكم، معكم زبون من "+NM+" — أبغى أطلب:"):("Hi "+NM+", I want to order:");
      var tail=ar?("الإجمالي: "+fmt(total())+" "+CUR):("Total: "+fmt(total())+" "+CUR);
      return head+"\\n"+lines.join("\\n")+"\\n"+tail;
    }
    function updateCheckout(){
      if(!csCheckout)return;
      if(PHONE){csCheckout.href='https://wa.me/'+PHONE+'?text='+encodeURIComponent(orderText());csCheckout.target='_blank';csCheckout.rel='noopener';if(csNote)csNote.textContent='';}
      else{csCheckout.removeAttribute('href');if(csNote)csNote.textContent=(lang()==='ar'?"يتفعّل مع رقم واتساب المقهى — نجهّزه لكم عند التفعيل":"Activates with the cafe's WhatsApp — set up on activation");}
    }
    function render(){
      var n=count(), t=total(), ar=lang()==='ar';
      document.querySelectorAll('.order-chip[data-add]').forEach(function(b){
        var id=b.getAttribute('data-id'), q=cart[id]?cart[id].qty:0, qs=b.querySelector('.oc-q');
        if(q>0){b.classList.add('incart');if(qs){qs.hidden=false;qs.textContent='×'+q;}}
        else{b.classList.remove('incart');if(qs){qs.hidden=true;}}
      });
      if(cbCount)cbCount.textContent=n; if(cbTotal)cbTotal.textContent=fmt(t);
      if(bar)bar.classList.toggle('show',n>0);
      document.body.classList.toggle('cart-active',n>0);
      if(csList){var keys=Object.keys(cart);
        if(!keys.length){csList.innerHTML='<div class="cs-empty">'+(ar?"سلّتك فاضية":"Your cart is empty")+'</div>';}
        else{csList.innerHTML=keys.map(function(id){var it=cart[id];var nm=ar?it.ar:it.en;
          return '<div class="cs-row"><span class="cs-nm">'+esc(nm)+'</span><span class="cs-pr">'+fmt(it.qty*it.price)+' '+CUR+'</span>'+
          '<span class="cs-step"><button type="button" data-dec="'+esc(id)+'" aria-label="-">−</button><span class="cs-q">'+it.qty+'</span><button type="button" data-inc="'+esc(id)+'" aria-label="+">+</button></span></div>';}).join('');}
      }
      if(csTotal)csTotal.textContent=fmt(t);
      updateCheckout();
    }
    document.addEventListener('click',function(e){
      var add=e.target.closest('.order-chip[data-add]');
      if(add){var id=add.getAttribute('data-id');
        if(!cart[id])cart[id]={ar:add.getAttribute('data-ar'),en:add.getAttribute('data-en'),price:parseFloat(add.getAttribute('data-price'))||0,qty:0};
        cart[id].qty++;save();render();
        if(!matchMedia('(prefers-reduced-motion: reduce)').matches){try{add.animate([{transform:'scale(1)'},{transform:'scale(.9)'},{transform:'scale(1)'}],{duration:220,easing:'cubic-bezier(.2,1.5,.4,1)'});}catch(e){}}
        return;}
      var inc=e.target.closest('[data-inc]'); if(inc){var i2=inc.getAttribute('data-inc');if(cart[i2]){cart[i2].qty++;save();render();}return;}
      var dec=e.target.closest('[data-dec]'); if(dec){var i3=dec.getAttribute('data-dec');if(cart[i3]){cart[i3].qty--;if(cart[i3].qty<=0)delete cart[i3];save();render();}return;}
    });
    var openBtn=document.getElementById('cartOpen'), closeBtn=document.getElementById('csClose'), back=document.getElementById('csBack');
    if(openBtn)openBtn.addEventListener('click',function(){if(sheet){sheet.classList.add('open');render();}});
    if(closeBtn)closeBtn.addEventListener('click',function(){if(sheet)sheet.classList.remove('open');});
    if(back)back.addEventListener('click',function(){if(sheet)sheet.classList.remove('open');});
    if(csCheckout)csCheckout.addEventListener('click',function(e){if(!PHONE){e.preventDefault();}});
    var lt2=document.getElementById('langToggle'); if(lt2)lt2.addEventListener('click',function(){setTimeout(render,0);});
    render();
  })();
