...
- **BUILD-TRACKER LAW (locked 2026-07-20 — Ibrahim loves tracking builds + the time they take):** every build/mission gets a live HTML tracker artifact (republish the SAME URL each update) that shows: (1) a big **live ELAPSED-TIME** number that counts UP from the real start (elapsed never lies — prefer it over a countdown estimate, which expires awkwardly and reads as "wrong"); (2) each wave/step wired to its **real timestamp** (finished-at clock time or a live-counting timer), green=done gold=now; (3) a plain-English "Right now" line; (4) a done/total progress bar. Must be simple English, big numbers, work OFFLINE (anchored to baked unix timestamps, ticks in-page), and keep running across refresh/chat-switch. Bake real epochs (from git commit times) so it's accurate, not guessed. Freeze the elapsed timer with a `doneAt` when finished. Keep him updated by republishing at each wave.
