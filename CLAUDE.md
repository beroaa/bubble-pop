...
- **Deliver** — passing menus are added to `menus/index.json`, committed, and (while push is blocked) delivered via SendUserFile. Redeploy the live tracker artifact to its SAME URL after each wave. **ClientOS first:** the user's Mac app (ClientOS) is their PRIMARY workstation — every delivery cycle regenerates `node menus/_kit/export-clientos.mjs` → `menus/clientos-import.json` and sends it to the user so their Mac session ingests it (this container cannot write to the Mac directly; the export file is the bridge). New menus reach ClientOS before anything else. **ClientOS web twin** (artifact `https://claude.ai/code/artifact/fd4b7aaf-930d-428f-9343-76b5cc3df19e`, scratchpad file `clientos-web.html`): pipeline board with stages/gift-links/copy-outreach/notes; state persists in the user's browser (localStorage) with an Export button. Its cafe data sits between `<<<DATA-START>>>`/`<<<DATA-END>>>` markers — update the DATA block and republish the SAME file path each cycle.

**ClientOS Mac app internals** (learned from the `.app` bundle): it's a compiled React/Vite app served on localhost (login crypto needs a secure context, so `file://` won't work) in a Chrome app-window, data stored in localStorage under a dedicated Chrome profile (`~/Library/Application Support/ClientOS-Chrome`). It has **Export data** (downloads a dated JSON backup of accounts/clients/ranks/XP) and **Import backup** which **OVERWRITES EVERYTHING**. The `.app` zip contains only code, never the user's cafe data. **To add menus without wiping the user's HQ: user clicks Export data → sends the JSON → we MERGE our menus into THAT backup (preserve their accounts/clients/ranks/XP, match the exact schema in their export, upsert cafes by slug) → user clicks Import backup to load it.** NEVER hand the user a fresh import file built from scratch — Import overwrites, so it would erase their existing cafes. Merge into their real export, every time.

**Standing user wish:** every new menu should land in the Mac ClientOS app (their main workspace) automatically. **Hard limit:** this cloud container cannot reach the user's Mac / its Chrome localStorage — the only inputs are what they upload here. Zipping the `.app` NEVER carries their cafe data (it's just the launcher). Real solutions, in order of preference: (1) get their `Export data` JSON ONCE (ask for a screenshot to guide them to the button — do NOT ask them to zip the .app again), learn the exact schema, then wire ClientOS to auto-pull `clientos-import.json` from the web on launch (true auto-sync); (2) until then, merge into each export they send. Never guess/replace their real data (25 DMs of pipeline) — merge only.

**EXPERT DEFAULT (decided after the user got stuck on the Mac export 3×): the ClientOS WEB TWIN is the practical daily driver.** It is frictionless and I keep it current every build cycle (update its DATA block + republish the same artifact URL) — the user needs to do NOTHING. The Mac-app merge is opportunistic, NOT a blocker: only do it if/when the user provides a fresh `clientos-backup-*.json` export, and never nag them for it. The user's Mac pipeline (their real cafes + DMs) stays untouched and safe regardless. If they want the Mac merged, the lightest ask is "send one screenshot of your app and I'll point to the export button" — never a repeated instruction loop.
